...
int kbhit (void);
float setSlope (float);
float setAlpha (float);
float AutoAlphaMs (float, float, float);	//  Converts  Alpha angle to ms for -- Auto Controller
...
//  cout << "alpha calibration on? y/n" << endl;
////  cin >> cal_a;
//  cout << "beta calibration on? y/n" << endl;
//  cin >> cal_b;
...
0      l_gz /= 2.0 * (float) NUM_IMU_SAMPLES;
...
//------------------------------ Calibration -------------------------------------------------- 
      ...
      if (c == 'q')
	{
	  ...
	  //printf("l_ca_ms pressed:%f \n",l_ca_ms);
	  Output (l_ca_ms, l_cb_ms);
...
setAlpha (float alpha)
{
  ...
  printf ("Alpha Calibration : Enter 1 to increemt , 2 to decreement\n");
  scanf ("%d", &input);
...
AutoBetaMs (float beta)
{
  ...
  // printf("beta_d:%f\n",beta_d);
  float mb_ms = 0;		//0.016;              //0.013;//0.0214;
  float cb_ms = 1.46;
...
float
AutoAlphaMs (float c, float m, float alpha)
{
  // float ca_ms = 1.45;
...
//   ma_ms = 0.0125;//0.0117;           // 0.015;
  //  ca_ms = 1.45;//1.44;

  float a_ms = m * alpha_d + c;
  return (a_ms);
...
/*
  if (ms3 > 2.0)
    ...
    ms3 = 1.0;
*/
  pwm1.set_duty_cycle (PWM_OUTPUT_1, 1.4);
