...
float TopThrustMs(float);
float BottomThrustMs(float);
float PeriodToMs(int);
float MsToTop(float);
float MsToBottom(float);
MatrixXd RotX(float); 
MatrixXd RotY(float);
MatrixXd RotZ(float);
MatrixXd Jaco(float,float,float); 
float AutoThrottleBottom(float,float);
float  AutoThrottleTop(float,float);
char *datetime(void);
...
float               g_k3 = 0.45;//.6908;//1.1931;       // alpha                            //*************************  -- P gain
float               g_k4 = 0.04;//0.005;//1.0*0.01;//.2;//0.67*0.3709;  // alpha           //*************************   -- D gain
float               g_k5 = 0.45;//.6908;//1.1931;//1.0; // beta                            //*************************   -- P gain
float               g_k6 = 0.04;//0.005;//1.0*0.01;//.2;//0.67*0.3709;  // beta            //*************************   -- D gain
...
//double ref_lat  =  pos_data[1]/10000000;
//double ref_long =   pos_data[2]/10000000;
...
*/
      ...
      g_k10 = 0;//`0;
                ...
                //position_des_kif(1) = (float) (l_periodAlphaAngle - 1492) / 400.0 * 0.1;
...
//----------- Auto Throttle ----------------------------------------------------------------
        if(l_switchSE<1500){
      float ms_top = PeriodToMs(l_periodRotorSpeed);
      float t_0_top = 1.400;//MsToTop(ms_top);
        //printf("%f\t%f\n",t_0_top,ms_top);
      float ms_bot = PeriodToMs(l_periodRotorSpeed);
      float t_0_bot = 1.400;//MsToBottom(ms_bot);
        //printf("t0_bot:%f\tms_bot%f,t0_top:%f\tms_top:%f\n",t_0_bot,ms_bot,t_0_top,ms_top);
      float thrust_top    = t_0_top + g_k1*(l_z-g_zd)+g_k2*l_z*dt;
      float thrust_bottom = t_0_bot + g_k1*(l_z-g_zd)+g_k2*l_z*dt;
        //float ms_3 = TopThrustMs(thrust_top);
        //float ms_4 = BottomThrustMs(thrust_bottom);
        //printf("%f\t%f\n",thrust_top,thrust_bottom);
      float ms_3 = AutoThrottleTop(thrust_top, thrust_bottom); //********************************************************* write the function for this
     float ms_4 = AutoThrottleBottom(thrust_bottom, thrust_top);
        //printf("%f\n",g_az);
     // printf("l_z:%f\tg_zd:%f\tdiff:%f\n",l_z,g_zd ,g_zd-l_z);
//printf("%f\t%f\n",ms_3,ms_4);
//printf("%f\n",thrust_top);
...
/*
        ...
        float c2 = 1.425;
        float thrust;
        if(l_periodRotorSpeed < 1250){
        thrust = m1*l_periodRotorSpeed + c1;
        }
        else{
        thrust = m2*l_periodRotorSpeed + c2;
        }
...
//strftime(filename, sizeof(filename), "%Y-%m-%d_%H:%M:%S", time);
...
float TopThrustMs(float thrust_top){

float mtop = 0.00012;
float ctop = 0.8032;
                                                                                                                                                                                                                           float ms_top = mtop*thrust_top+ctop;
...
float BottomThrustMs(float thrust_bot){

float mbot = 0.000094;
float cbot = 0.92486;

float ms_bot = mbot*thrust_bot+cbot;
...
float AutoThrottleTop(float thrustTop,float thrustBottom){
float mtop = 473.7829;
float mbot =  468.9340;
//thrustTop = thrustTop*1000;
//float ktop =  0.0016;

float k = 2.6733;
float pwm_desired = sqrt(thrustTop+1/k);

/*
float k1t = 0.0016;
float k2t = -0.2626;//0.6487;
float omega_desired = k1t*sqrt(thrustTop/mtop)+ k1t*k2t*sqrt(thrustTop/mtop)*sqrt(thrustBottom/mbot);

float pwm_desired = omega_desired/k1t;
*/
return(pwm_desired);

}

float AutoThrottleBottom(float thrustBottom, float thrustTop){
//thrustBottom = thrustBottom*1000;
float mbot =  468.9340;
float mtop = 473.7829;
//float kbot =  0.0014;

float k = 2.6733;
float pwm_desired = sqrt(thrustBottom+1/k);

/*
float k1b = 0.0014;
float k2b = 0.6162;//-1.0476;
float omega_desired = k1b*sqrt(thrustBottom/mbot)+k1b*k2b*sqrt(thrustTop/mtop)*sqrt(thrustBottom/mbot);
float pwm_desired = omega_desired/k1b;
*/

return(pwm_desired);
}

float PeriodToMs(int Period){
        float m =  0.001202;      // 0.001;
        float c = -0.3174;//0.3064;//-0.3126;     //-0.15;
       float ms = m * Period + c;
}

float MsToTop(float ms){
float m_topms = 1303.03;//1683.33;//893.94;
float c_topms = -1074.03;//-1942.93;//-893.94;

float T_0_top = m_topms*ms + c_topms;

return (T_0_top);

}
float MsToBottom(float ms){

float m_botms = 1187.88;//1636.36;//757.56;
float c_botms = -900.8804;//-1926.359;//-757.56;

float T_0_bot = m_botms*ms + c_botms;
...
//printf("Do you want to log the data? y/n \n");
//scanf("%c",&g_status);
...
/*
  ...
  while ((parameter = getopt (argc, argv, "y:n")) != -1)
    {
      switch (parameter)
        {
        case 'y':
